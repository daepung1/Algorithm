...
def solution(s):
    ...
    for i in range(len(temp)-1):
        temp_set[len(temp[i])]=temp[i]

    temp_keys=list(temp_set.keys())
    temp_keys=sorted(temp_keys)
    for i in temp_keys:
        for l in range(len(temp_set[i])):
            
            if temp_set[i][l]!=',':
                temp_str=temp_str+temp_set[i][l]
            else:
                if temp_str not in check:
                    answer.append(int(temp_str))
                    check.append(temp_str)
                    temp_str=''
                    
                else:
                    temp_str=''    
            if l==len(temp_set[i])-1:
                if temp_str not in check:
                    answer.append(int(temp_str))
                    check.append(temp_str)
                    temp_str=''                    
